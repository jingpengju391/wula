...
import { GroupItem, SessionType } from "../../../utils/open_im_sdk/types";
...
const GroupCard: FC<GroupCardProps> = ({ draggableCardVisible, info, close }) => {
  ...
  const nextStep = () => {
    if (inGroup) {
      events.emit(TOASSIGNCVE, info.groupID, SessionType.Group);
    } else {
      setStep("send");
    }
  };
  ...
  const InfoTitle = () => {
    return (
      <>
        ...
        <MyAvatar src={info.faceURL==="" ? group_icon:info.faceURL} size={42} />
      </>
    );
  };

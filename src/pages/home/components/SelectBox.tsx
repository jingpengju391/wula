...
const SelectBox: FC<SelectBoxProps> = ({ memberList, friendList, groupList, showGroup = false, onSelectedChange }) => {
  ...
  const isSelectMember = rs.memberList.length > 0;
  const isSearchEmpty = rs.searchList.length === 0;
      ...
      <div className="select_box">
        <div className="select_box_left">
          ...
          {isSelectMember ? (
            ...
            // @ts-ignore
            rs.searchList.map((s) => <LeftSelectItem key={s.userID ?? s.groupID} item={s} />)
          ) : rs.searchText !== "" ? (

...
export const parseTime = (timestamp: number, type: 0 | 1) => {
  const arr = formatDate(timestamp);
...
export const fileToBase64 = (file: File | RcFile) => {
  return new Promise((resolve, reject) => {
...
export const downloadFileUtil = (filePath: string, filename: string, msgID: string) => {
  if (window.electron) {
    window.electron.download(filePath + "?msgid=" + msgID);
    return;
  }
  const linkNode = document.createElement("a");
  linkNode.download = filename;
  linkNode.style.display = "none";
  linkNode.href = filePath + "?msgid=" + msgID;
...
export const sec2Time = (seconds: number) => {
  ...
  if (theTime1 > 0) {
    result = "" + parseInt(theTime1 as unknown as string) + "分钟" + result;
  }
  if (theTime2 > 0) {
...
export const switchUpload = async (
  uploadData: UploadRequestOption,
  fileType: minioUploadType = minioUploadType.file,
  isNomal: boolean = false,
  snapShot?: RcFile,
  onProgress?: (progress: number) => void
) => {
  if (OBJECTSTORAGE === "minio") {
    return minioUpload(isNomal ? ({ file: uploadData } as any) : uploadData, fileType, undefined, onProgress);
  } else {
...
export const diffMemo = (prev: any, next: any, diffKey: string[]) => {
  const flags: boolean[] = [];
  diffKey.forEach((key) => flags.push(JSON.stringify(prev[key]) === JSON.stringify(next[key])));
...
export const genAvatar = (str: string, size: number) => {
  ...
  ctx!.fillText(str.slice(str.length > 1 ? -2 : -1), size / 2, size / 2);
